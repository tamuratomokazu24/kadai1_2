...
private:
	struct Node {
		...
		* @param[in] preNode １つ前方のNodeを指すポインタ
		* @param[in] nextNode １つ後方のNodeを指すポインタ
		* @param[in] scoreData ノードが持つ値
		*/
		Node(Node* preNode, Node* nextNode, const ScoreData& scoreData);
	};
...
public:
	...
	* @param[in] scoreData 挿入する値
	*
	* @return bool	true:正常にデータを挿入することができた\n
	*				false:データの挿入に失敗した\n
	*/
	bool insert(Iterator iterator,const ScoreData& scoreData);
	...
	*				false:データの挿入に失敗した\n
	*/
	bool insert(ConstIterator cIterator, const ScoreData& scoreData);
